"""
Utility: Tạo Discord Embeds dùng chung
"""
...
def base_embed(
    ...
    embed.set_footer(text="Minecraft Bot")
    return embed
...
def server_status_embed(
    ...
    if online:
        ...
        embed.add_field(name="Địa chỉ", value=f"`{ip}`", inline=True)
        embed.add_field(
            name="Người chơi",
        ...
        embed = base_embed(
            ...
            description=f"Server `{ip}` hiện không hoạt động.",
            color_key="offline",
        )
...
def aternos_status_embed(status: str, server_ip: str = "") -> discord.Embed:
    status_map = {
        ...
        "stopping":  ("Server đang tắt...",        "warning", "Đang tắt"),
        "error":     ("Lỗi server",                "error",   "Lỗi"),
    }
    title, color_key, status_text = status_map.get(
        status.lower(),
        (f"Trạng thái: {status}", "info", status),
    )
